...
export class SyncActivityRepository {
  ...
  async create(start: string, end: string): Promise<string> {
    const result = await this.database.query<{ id: string }>(
      `INSERT INTO sync_activities(owner_id,requested_start,requested_end)
  ...
  async lastCheckpoint(): Promise<string | null> {
    const result = await this.database.query<{ checkpoint: string }>(
      `SELECT checkpoint FROM sync_activities WHERE owner_id=$1 AND state='succeeded'
       AND checkpoint IS NOT NULL ORDER BY finished_at DESC LIMIT 1`,
      [LOCAL_OWNER_ID],
    );
  ...
  async update(id: string, update: Record<string, unknown>): Promise<void> {
    await this.database.query(
      `UPDATE sync_activities SET state=COALESCE($3,state),scanned_count=COALESCE($4,scanned_count),
       created_inbox_item_count=COALESCE($5,created_inbox_item_count),
       skipped_processed_count=COALESCE($6,skipped_processed_count),checkpoint=COALESCE($7,checkpoint),
       failure_message=COALESCE($8,failure_message),finished_at=COALESCE($9,finished_at)
