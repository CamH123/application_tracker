...
export class GmailRepository {
  ...
  async save(encryptedToken: string, gmailAddress: string): Promise<void> {
    await this.database.query(
      `INSERT INTO gmail_connections(owner_id,encrypted_refresh_token,gmail_address)
       VALUES($1,$2,$3) ON CONFLICT(owner_id) DO UPDATE SET
       encrypted_refresh_token=EXCLUDED.encrypted_refresh_token,gmail_address=EXCLUDED.gmail_address,
...
export class ProcessedMessageRepository {
  ...
  async add(
    ...
    await this.database.query(
      `INSERT INTO processed_gmail_messages(owner_id,gmail_message_id,classification_outcome)
       VALUES($1,$2,$3) ON CONFLICT(owner_id,gmail_message_id) DO NOTHING`,
