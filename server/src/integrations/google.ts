...
import type { GmailRepository } from "../repositories/gmail.js";
import type { GmailGateway } from "../sync/sync.js";
...
export class GoogleOAuthService {
  ...
  async complete(code: string, state: string): Promise<void> {
    ...
    const gmail = this.createGmail(auth);
...
const bodyText = (payload?: gmail_v1.Schema$MessagePart): string => {
  ...
  if (plain) return bodyText(plain);
  return (
...
const gmailDateRangeQuery = (start: string, end: string): string => {
  ...
  const inclusiveStart = Math.floor(startBoundary.valueOf() / 1000) - 1;
  const exclusiveEnd = Math.floor(endBoundary.valueOf() / 1000);
  return `after:${inclusiveStart} before:${exclusiveEnd}`;
};
