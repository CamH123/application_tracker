...
describe("Application projections", () => {
  ...
  it("keeps an interview stage after an earlier completed assessment", () => {
    expect(
      projectApplicationState([
        event("1", "assessment_completed", "2027-02-01", "2027-02-01T10:00:00Z"),
  ...
  it("recalculates from the remaining sequence when the latest event is deleted", () => {
    ...
    expect(projectApplicationState(events.slice(0, 1)).currentStatus).toBe("Interviewing");
  });

  it("requires explicit acknowledgement for a non-terminal event after completion", () => {
    expect(requiresReopenAcknowledgement([event("1", "rejected", "2027-02-03", "2027-02-03T09:00:00Z")], "interview_scheduled")).toBe(true);
    expect(requiresReopenAcknowledgement([event("1", "rejected", "2027-02-03", "2027-02-03T09:00:00Z")], "withdrawn")).toBe(false);
  });
});
...
describe("duplicate normalization", () => {
  it("normalizes case and whitespace without removing meaningful punctuation", () => {
    ...
    expect(normalizeRoleTitle("  Software  Engineer, Intern ")).toBe("software engineer, intern");
  });
});
